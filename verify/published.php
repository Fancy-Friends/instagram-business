...
/*
 * Instagram for Business — the published Composer package.
 *
...
if (! is_file($autoload)) {
    ...
    fwrite(STDERR, '    composer require particle-academy/instagram-business-php'.PHP_EOL);
    exit(2);
}
...
foreach ($goldens as $golden) {
    ...
    $fake = new FakeValues(FakeValues::seedForCall('instagram_business', $operation, $config));
    $faked = InstagramBusinessFaker::respond($operation, ['config' => $config, 'fake' => $fake]);
