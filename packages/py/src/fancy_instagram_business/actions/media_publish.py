...
def body(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if config.get("igUserId") is None or config.get("igUserId") == "":
        raise ConnectorConfigError(
            "media_publish: \"igUserId\" is required (Instagram user ID)."
        )

    if config.get("creationId") is None or config.get("creationId") == "":
        raise ConnectorConfigError(
            "media_publish: \"creationId\" is required (Creation ID)."
        )

    out: dict[str, Any] = {}
    _value = config.get("creationId")
    if _value is None or _value == "":
        raise ConnectorConfigError("media_publish: \"creationId\" is required.")

    out["creation_id"] = str(_value)
