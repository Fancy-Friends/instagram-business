...
def _media_container_create(config: dict[str, Any], fake: FakeValues) -> Any:
    return {
        "id": "17889455560051444",
    }


def _media_publish(config: dict[str, Any], fake: FakeValues) -> Any:
    return {
...
def respond(operation: str, request: dict[str, Any]) -> Any:
    ...
    if operation == "media_container_create":
        return _media_container_create(config, fake)

    if operation == "media_publish":
        return _media_publish(config, fake)
    ...
    raise ValueError(
        f'instagram_business: no fake response is defined for "{operation}". '
