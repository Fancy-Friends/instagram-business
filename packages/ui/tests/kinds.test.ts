...
import { INSTAGRAM_BUSINESS_KINDS } from "../src/index.js";
...
test("the registry names are exactly what the manifest says", () => {
  assert.deepEqual(INSTAGRAM_BUSINESS_KINDS.map((kind) => kind.name), [
    "@particle-academy/instagram_business_media_container",
    "@particle-academy/instagram_business_media_publish"
  ]);
});
